...
{
	//These values are taken from the PayPal's dashboard and they will be used by the Auth2.0 authentication
	...
	public void authorizePayment(BookOrder order) throws ServletException, IOException 
	{
		...
		//Dont need this here anymore - I will create this object in the getTransaction() method...
	...
	private RedirectUrls getRedirectsURLs()
	{
		...
		//The cancel URL points to the Servlet that forward to the shopping cart jsp page
		String cancelUrl = baseURL.concat("/view_cart"); 
		
		//The returnUrl points to the Servlet that forward to the review payment.jsp page
	...
	private ShippingAddress getRecipientInformation(BookOrder order)
	{
		...
		/*
		 * Read the recipient information from the BookOrder object
		 *  into the shippingAddress object by chaining code
		 */
		shippingAddress.setRecipientName(order.getFirstname() + " " + order.getLastname())
